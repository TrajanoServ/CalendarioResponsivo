...
function verdeSeSegundaSexta(event: EventApi) {
    const diaSemana = event.start.getDay(); // Obtém o dia da semana (0 = domingo, 6 = sábado)
    if (diaSemana >= 1 && diaSemana <= 5) { // Segunda a sexta
        return '<span style="background-color: green;">' + event.title + '</span>';
    } else {
        return event.title;
    }
}
